create or replace function public.record_level_play(p_level_id uuid)
...
begin
  ...
  insert into public.play_events(user_id, level_id) values(auth.uid(), p_level_id);
end;
$$;
...
begin
  ...
  if p_kind = 'art' then
    select coalesce(jsonb_agg(item order by
      ...
      case when p_sort = 'top' then (item->>'plays')::integer end desc,
...
grant execute on function public.record_level_play(uuid) to anon, authenticated;
grant execute on function public.browse_gallery(text, text) to anon, authenticated;

create index if not exists play_events_level_idx on public.play_events(level_id);
